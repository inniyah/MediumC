...
void getstmtlist(entry *fptr)
{
    ...
    if(!match("(")) error("missing open paren");
                                 ...
                                 /* init params table */
    fptr->further=(list *)calloc(1,sizeof(list));
    ...
    if(legalname) {
        if(findloc(loctab,name,ALL))
            multidef(name);
                                 ...
                                 /* add to local global */
            p=addsymbol(loctab,name,AUTO,ptyp->type,PUBLIC,0);
            p->size=ptyp->size;
            p->offset=argstk;
                                 /* add to a further in global */
            p=addsymbol(table,name,AUTO,ptyp->type,PUBLIC,0);
            p->size=ptyp->size;
            p->offset=argstk;
